...
class BaseDataset(Dataset):
    ...
    '''
    def __init__(self, anns, image_dir, scale=1, resize_size=None, center_crop=1, use_aug=False, num_classes=19, pseudo_dir=None, target=False):
        self.target = target
        if self.target:
            self.image_list,_ = read_anns(anns, image_dir,target)
            ...
            self.image_list, self.label_list = read_anns(anns, image_dir,target)
            self.transforms = trans
            self.scale = scale
            self.normalize = {
                "mean": [0.485, 0.456, 0.406],
    ...
    def aug(self, image, label):
        target = self.target
        return seg_aug(image, label, target)
